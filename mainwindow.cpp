...
void MainWindow::showProduitsInterface() {
    ...
    if (m_statsWidgetProduit) {
        ...
        if (isListPage) {
            rafraichirTableauProduit(); // Cette fonction appelle maintenant highlightLowStockProducts()
            rafraichirStatistiquesProduits();
            verifierStockFaible();
        }
    }
...
void MainWindow::initialiserAPI()
{
    ...
    // Si la connexion échoue, activer le mode simulation
    connect(m_apiRemises, &ApiRemises::connexionTestee, this, [this](bool succes) {
        if (!succes) {
            qDebug() << "🔧 Activation du mode simulation";
            m_apiRemises->setModeSimulation(true);

            // Avertissement utilisateur
            QMessageBox::information(this, "Mode Simulation",
                                     "L'API de remises n'est pas disponible.\n"
                                     "Mode simulation activé - Les remises seront calculées localement.\n"
                                     "Fonctionnalités disponibles :\n"
                                     "• Calcul automatique basé sur la quantité\n"
                                     "• Règles : 0-9=0% | 10-19=5% | 20-49=10% | 50-99=15% | 100+=25%");
        }
    });
