...
Gestion_Fournisseur::Gestion_Fournisseur()
    : m_id(0), m_nomEntreprise(""), m_personneContact(""), m_adresse(""),
    m_telephone(0), m_mail(""), m_categorie(""), m_details(""), m_noteFiabilite(0.0f)
{}
...
bool Gestion_Fournisseur::setPersonneContact(const QString &personne) {
    m_personneContact = personne.trimmed().left(50);
    return true;
}

bool Gestion_Fournisseur::setAdresse(const QString &adresse) {
    m_adresse = adresse.trimmed().left(100);
    return true;
}
...
bool Gestion_Fournisseur::setDetails(const QString &details) {
    m_details = details.trimmed().left(100);
    return true;
}
                                             ...
                                             float noteFiabilite, QString &erreurMessage)
{
    ...
    Connection c;
    if (!c.createconnect()) { erreurMessage = "Connexion échouée !"; return false; }
    ...
    QSqlQuery query(db);
    query.prepare(
        "INSERT INTO FOURNISSEUR (ID_FOURNISSEUR, NOM_ENTREPRISE, PERSONNE_CONTACT, ADRESSE, "
        "NUM_TEL, MAIL, CATEGORIE, DETAILS, NOTE_FIABILITE) "
        "VALUES (:id, :nom, :personne, :adresse, :tel, :mail, :categorie, :details, :note)"
        );

    query.bindValue(":id", id);
    query.bindValue(":nom", nomEntreprise);
    query.bindValue(":personne", personneContact.left(50));
    query.bindValue(":adresse", adresse.left(100));
    query.bindValue(":tel", telephone);
    query.bindValue(":mail", mail);
    query.bindValue(":categorie", categorie);
    query.bindValue(":details", details.left(100));
    ...
    if (query.exec()) {
        ...
        erreurMessage = "Erreur SQL : " + query.lastError().text();
        return false;
    }
}
...
QList<Gestion_Fournisseur> Gestion_Fournisseur::chargerTousLesFournisseurs()
{
    ...
    QSqlDatabase db = QSqlDatabase::database();
    if (!db.isOpen()) return fournisseurs;

    QSqlQuery query(db);
    query.prepare(
        "SELECT ID_FOURNISSEUR, NOM_ENTREPRISE, PERSONNE_CONTACT, ADRESSE, NUM_TEL, "
        "MAIL, CATEGORIE, DETAILS, NOTE_FIABILITE FROM FOURNISSEUR ORDER BY NOM_ENTREPRISE"
        );
                                              ...
                                              float noteFiabilite, QString &erreurMessage)
{
    ...
    QSqlQuery query(db);
    query.prepare(
        "UPDATE FOURNISSEUR SET NOM_ENTREPRISE = :nom, PERSONNE_CONTACT = :personne, "
        "ADRESSE = :adresse, NUM_TEL = :tel, MAIL = :mail, CATEGORIE = :categorie, "
        "DETAILS = :details, NOTE_FIABILITE = :note WHERE ID_FOURNISSEUR = :id"
        );
...
bool Gestion_Fournisseur::isValidNom(const QString &nom) {
    return nom.trimmed().length() >= 2 && nom.trimmed().length() <= 50;
}
...
bool Gestion_Fournisseur::isValidMail(const QString &mail) {
    ...
    return regex.match(mail).hasMatch() && mail.length() <= 50;
}
